...
****debug 和正式版能并存。** debug 包名带 `.debug` 后缀，桌面上叫「moodiary debug」，
和从 Release 装的正式版各有各的日记，装 debug 不用先卸载正式版。两条注意：
debug 的 `UPDATE_REPO` 是空的，「检查更新」永远显示已是最新（包名不同，下回来的
正式 APK 会装成第二个 App 而不是升级）；MCP 端口写死 8765，两个版本同时开会撞。
换了包名之后第一次装，HyperOS 会弹「USB 安装提示」要手点「继续安装」，之后同包名
更新就不问了。
...
- `PlaceSource.nearby()` 还是桩：「附近」那种带距离的 POI 搜索要高德/百度的 key，
  ...
  后端时退回坐标文本）
- 「我的」顶上的名字（设计稿写死的「念念」）点一下能改，存在 prefs（`ProfileSettings`，
  最长 12 字，留空回落「念念」，头像是名字第一个字）。设计稿里它不可点
